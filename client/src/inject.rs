    ...
    fn local_pe_exec(&mut self, param: String, export: String) -> Result<(), String> {
        ...
        unsafe {
            if self.is_dll {
                ...
                let result = func_dll(HINSTANCE(address as *mut c_void), DLL_PROCESS_ATTACH, null_mut());
                if result == false {
                    return Err("[!] DllMain function failed".to_string());
                }
                ...
                std::thread::spawn(move || {
                    if func() == false {
                        Err::<(), _>("[!] Main function failed".to_string()).unwrap();
                    }
                });
            }
            Ok(())
    ...
    fn realoc_image(&self, address: *mut c_void) -> Result<(), String> {
        unsafe {
            ...
            while (*base_relocation).VirtualAddress != 0 {
                let mut base_entry = base_relocation.offset(1) as *mut BaseRelocationEntry;
                let block_end = (base_relocation as *mut u8).offset((*base_relocation).SizeOfBlock as isize) as *mut BaseRelocationEntry;
